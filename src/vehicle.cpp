    ...
    */
    ...
    if (get_vehicle_ahead(predictions, lane, vehicle_ahead)) {
        ...
        if (get_vehicle_behind(predictions, lane, vehicle_behind)) {
            ...
            float max_velocity_in_front = (vehicle_ahead.s - this->s - this->preferred_buffer) + vehicle_ahead.v - 0.5 * (this->a); // Equation: d = d0+ vt -0.5a(t^2)
            new_velocity = min(min(max_velocity_in_front, max_velocity_accel_limit), this->target_speed);
			float new_accel =  this->v - new_velocity;
			...
			{
				new_velocity -= this->max_acceleration;
    ...
    new_position = this->s + new_velocity + new_accel / 2.0; // Equation distance = s + vt + 0.5a(t^2)
